...
def shuffle_and_split(traind):
    ...
    num_ftrs = len(X[0])
    #needed for decision trees
    flist = [str(y) for y in range(num_ftrs)]
    return {'data': X, 'target': y, 'q_id': q_ids, 'feature_names': flist}
...
def enhance_data(datadict):
    ...
    for k, v in temp_dict.items():
        #print k,v
        vals = set(v)
        #if there are 10 or fewer values and they are ints, keep to treat as categorical feature
        if 0 < len(vals) <= 10 and all_ints(vals):
            temp_dict[k] = list(vals) #change it back to a list for mutability
            continue
        #if there are floats (which could also be ints), don't keep all the vals
        is_numeric = False
        for x in vals:
            if is_float(x):
                temp_dict[k] = 'numeric'
...
#Utility class to optionally use
class Counter(dict):
    """
